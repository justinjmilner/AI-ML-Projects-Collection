...
def load_data(data_dir):
    ...
    # Open each directory in root folder
    for i in range(NUM_CATEGORIES):
        ...
        for file in os.listdir(category_dir):
            ...
            img = cv2.imread(file)
            assert img is not None, "imread error"
...
def get_model():
    ...
    # C1 block
    model.add(tf.keras.layers.Conv2D(30, (5, 5), activation="relu", input_shape=(IMG_WIDTH, IMG_HEIGHT, 3)))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU(alpha=0.05))
    model.add(tf.keras.layers.Conv2D(30, (5, 5), activation="relu"))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU(alpha=0.05))
